...
User options are to search for products, browse and filter products, login to an existing account, register for a new account, or search for users.

## Buttons on top center of home page. 
Once the user is logged in, they can also access their cart, their reviews, and their inventory (if they are a seller). A user has the option of becoming a seller, in which they will be able add and edit inventory, see and update status of buyer orders, see reviews about their store, and see analytics of their store.

## All Products button. 

To search for products, enter a keyword into the search bar. This will display products that contain the keyword either in their product description or product name. Once on the product browse page, the user can filter by category, max price, and again, keyword. The product browse page lists important product details (such as name, description, price, etc.) and average product review. The user can click on a product name to see that product's detail page. 

On the product details page, the user can see all product information, all product reviews, and all product sellers. They are also able to add a certain quantity of that product to their cart based on seller. This quantity cannot be <= 0 and cannot be more than the seller has available. The user is also able to add a review to the product on a given product details page.

## Cart button.
Once a product has been added to the user's cart, they can see that item, its seller, its cost, and its status in their cart. They have the option to delete an item from their cart, change the status of an item in their cart (in cart or save for later), change quantity of items in cart, or submit order. Items cannot be purchased if: the user does not have enough money or the quantities in cart are no longer available from the selected seller. 

Once an order has been submitted, the user will see a confirmation of their order and the receipt of their order. Their account balance will be decremented by the total cost of the submitted order.

From a seller's perspective, their account balance will be incremented and their inventory quantity will be decremented by the submitted order. They will be able to access their store and see the order information as well as change the order status between fulfilled and in progress.

## User Lookup button.
Allows users logged in to find reviews and information made by other users.

## Your Reviews button.
Allows users logged in to see all the reviews written by themselves.

## Your Store button.
Shows a User's inventory, orders, analytics, and reviews for that user's store.
